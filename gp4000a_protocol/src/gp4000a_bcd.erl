...
decode({ _,Binary} = Value) ->
  decode(Value, bit_size(Binary)).

decode({unsigned, BCD}, Size) ->
    decode_unsigned(BCD, min(Size,bit_size(BCD)), <<>>);
...
decode_unsigned(<<N1:4, Num/bits>>, Size, Text) when N1 < 10 ->
    decode_unsigned(Num, Size-4, <<Text/bits, (N1+16#30)>>).
